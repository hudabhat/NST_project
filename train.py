...
def parse_arguments():
    ...
    parser.add_argument('--content_dir', type=str,default='C:\\Users\\hudab\\OneDrive\\Desktop\\NST\\content_data',help='Location of content dataset')
    parser.add_argument('--style_dir',type=str,default='C:\\Users\\hudab\\OneDrive\\Desktop\\NST\\style_data',help='Location of style dataset')
    parser.add_argument('--vgg',type=str,default='C:\\Users\\hudab\\OneDrive\\Desktop\\NST\\vgg_normalised.pth',help='Location of pre-trained VGG')
...
def main():
    ...
    with open(save_dir / 'args.txt', 'w') as args_file:
        for key,value in vars(args).items(): #vars to convert args to dictionary
    ...
    for epoch in range(args.epochs):
        ...
        for content_batch, style_batch in progress_bar:
            ...
            running_loss += loss.item() # item will give value of tensor
        ...
        if (epoch+1)%args.log_interval == 0:
            tqdm.write(f'Iteration {epoch+1}: Loss:{running_loss:4f}, Content Loss: {running_closs:4f}, Style Loss: {running_sloss:4f}')
        ...
        if (epoch+1)%args.save_interval == 0:
            ...
            #save the output of last batch as well
            with torch.no_grad():
                output = torch.cat([content_batch,style_batch,g],dim=0)
                save_image(output, save_dir / f'output {epoch+1}.png', nrow=args.batch_size)
